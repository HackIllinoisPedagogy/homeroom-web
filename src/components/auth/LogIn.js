...
function LogIn() {
    ...
    return (
        <div style={{
            ...
            >
                <div className="w-full justify-center flex content-start flex-wrap " ><Logo width={50} height={50} /><span className=" p-3 align-bottom ml-2 text-2xl font-bold">
                    Pedagogy
                </span></div>

            </div>
            <div className="w-full justify-center flex flex-col  p-3" style={{ 'height': '90%', }}>
                <span className="w-1/4 self-center font-bold text-3xl text-center mb-10">
                    Welcome back!
                </span>
                <form className="w-1/4 self-center">
                    <div className="flex items-center py-2">
                        <input
                            className="appearance-none h-12 block w-full bg-gray-200 text-gray-700 rounded py-3 px-4 mb-1 leading-tight focus:outline-none"
                            type="email" placeholder="Email" aria-label="email" value={email} onChange={event => setEmail(event.target.value)} />
                    </div>
                    <div className="flex items-center py-2">
                        <input
                            className="appearance-none h-12 block w-full bg-gray-200 text-gray-700 rounded py-3 px-4 mb-1 leading-tight focus:outline-none focus:bg-white"
                            type="password" placeholder="Password" aria-label="password" value={password} onChange={event => setPassword(event.target.value)} />
                    </div>
                </form>
                <button
                    style={{ background: '#7754F8', borderColor: '#7754F8' }}
                    className="self-center h-12 w-1/4 rounded bg-purple-500 hover:bg-purple-700 mt-3 py-1 px-2 border-4 text-sm text-white hover:shadow-inner"
                    onClick={() => {
                ...
                <button
                    className="my-3 self-center h-12 w-1/4 rounded bg-orange-500 hover:bg-orange-700 border-orange-500 hover:border-orange-700 py-1 px-2 border-4 text-sm text-white hover:shadow-inner">
                    No account? Sign up!
                </button>
                {errorDiv}
            </div>
