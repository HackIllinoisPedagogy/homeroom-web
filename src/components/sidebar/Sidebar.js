...
class Sidebar extends Component {
    ...
    createClassName = (name) => {
        this.setState({createClassName: name});
    }
    ...
    componentDidUpdate = async () => {
        if (this.props.user && !this.state.doc) {
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                ...
                                <div class="mt-2">
                                    <input
                                        ...
                                        onChange={(e) => this.createClassName(e.target.value)}
                                    />
                                    ...
                                    onClick={() => {
                                        ...
                                        db.collection("classes").doc(code + "").set(
                                            ...
                                            db.collection("users").doc(this.props.user.uid).update({
                                                classes: firebase.firestore.FieldValue.arrayUnion(code + "_" + this.state.createClassName)
                                            }).then(() => {
                        ...
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 mr-4 sm:text-left sm:w-full">
                                ...
                                <div class="mt-2">
                                    <input
                                        class="appearance-none block w-full bg-gray-200 text-gray-700 border rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white"
                                        id="grid-first-name" type="number" placeholder="Enter class code..." value={this.state.createClassName} onChange={e => this.setState({createClassName:e.target.value})}/>
                    ...
                    <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                        <span class="flex w-full rounded-md shadow-sm sm:ml-3 sm:w-auto">
                            <button type="button"
                                    class="inline-flex justify-center w-full rounded-md border border-transparent px-4 py-2 bg-p-purple text-base leading-6 font-medium text-white shadow-sm hover:bg-red-500 focus:outline-none focus:border-red-700 focus:shadow-outline-red transition ease-in-out duration-150 sm:text-sm sm:leading-5"
                                    onClick={() => {
                                        db.collection("classes").doc(this.state.createClassName).get().then(doc => {
                                            if(doc.exists) {
                                                db.collection("users").doc(this.props.user.uid).update({
                                                    classes: firebase.firestore.FieldValue.arrayUnion(this.state.createClassName + "_" + doc.data().name)
                                                }).then(() => {
                                                ...
                                                alert("This class code is invalid");
                                            }
                                        })
                                    }}
                            >
    ...
    renderMainSidebar() {
        ...
        const classes = this.state.doc ? <div>{this.state.doc.classes}</div> : <div/>
        return (
            <div style={{width: '300px'}}
                ...
                <div class="flex max-w-xs p-4 bg-white">
                    <ul class="flex flex-col">
                        ...
                        <li class="my-px">
                            <span
                                class="flex font-medium text-sm text-gray-400 px-2 my-4 uppercase">My Assignments</span>
                        </li>
                        ...
                        <li class="my-px">
                            <a href="#"
                               class="flex flex-row items-center h-12 px-4 rounded-lg text-gray-600 hover:bg-gray-100">
                                ...
                                <span class="ml-3" onClick={() => this.logout()}>Logout</span>
                            </a>
                        </li>
                    </ul>
                </div>
