...
class Tutor extends React.Component {
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        let {solution} = prevState;
        if (solution !== this.state.solution) {
            this.loadSolution(this.state.solution);
        }
        if (!_.isEqual(this.props.problem.solution, prevProps.problem.solution)) {
            this.setState({
                solution: this.props.problem.solution,
                hint: <div/>,
            });
        }
    }
    ...
    loadSolution = (solution) => {
        let data = {"soln": solution};

        let query = Object.keys(data)
            .map(k => encodeURIComponent(k) + '=' + encodeURIComponent(data[k]))
            .join('&');

        fetch(this.serverUrl + "load_soln?" + query)
            .then(res => res.json())
            .catch(error => {
                console.log(error);
            });
        ...
        fetch(this.serverUrl + "get_hint?" + query)
            ...
            .then(
                (result) => {
                    ...
                    let h = (
                        <div id="hint-box"
                             className="w-full bg-white h-auto shadow-md rounded mt-5 p-5 w-1/5 overflow-x-auto">
        ...
        return (
            <div id="tutor" className="w-3/5">
                ...
                <form id="user-input" className="mt-5">
					<textarea id="user-state"
                              className="text-gray-700 focus:outline-none w-full h-full p-3 bg-white shadow-md rounded"
                ...
                <div id="button-container" className="w-full h-auto flex pt-5 justify-end">
                    <button id="submit-hint-request"
                            className="bg-custom-purple text-gray-100 w-1/5 shadow-md text-center h-8"
